...
and `ReferenceRotationToMidpointReflectionBoundary`.  Existing
`ReferenceMatchingLocalObstructionBoundary` and `FixedStarLocalObstructionBoundary`
packages also imply it, so the remaining task is to prove that non-circular
local obstruction/support-complement input from the raw default-base geometry.
Together this reference input and one endpoint input close the canonical
raw-action branch/A-fiber frontier.
