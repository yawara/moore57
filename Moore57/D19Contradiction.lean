...
* Section 4 の表現論側データ (`TraceCharacterData`) と寄与公式
  $a_1(r^d)=38+38N_d$ が得られる. Higman 跡公式部分は
  `TraceCharacterData.toTraceRepresentationData` の中で Lean 内の行列跡公式から導く.
...
従ってこの公理には `D_q`, `|D_q|≤2`, `Nd_lower`, 旧来の Higman 型整数等式を
含めず, 軌道列 `W` と Section 4 の表現論側入力データだけを残している.
-/
