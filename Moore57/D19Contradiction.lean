...
ただし Section 4 から最終計数に使う下界 `Nd_lower` 自体は, `TraceRepresentationData` と
寄与公式から Lean 内で証明する.
...
* `rotation_trace`: 非自明回転に対する Higman 型跡公式と有理表現の指標値の一致
...
`traceRep` と `a1_contribution` は Section 4 の入力を最終計数に接続するための
データである. ここから `Nd_lower` は下の定理 `Nd_lower` で証明するため,
具体構造のフィールドとしては仮定しない. -/
...
跡公式・表現論の算術補題から証明できる. -/
theorem Nd_lower (h : D19ConcreteHypotheses Γ) :
    ∀ d : ZMod 19, d ≠ 0 →
      8 ≤ ((Finset.univ : Finset (Fin 56)).filter (fun q => d ∈ h.D q)).card :=
  Nd_lower_of_trace_representation h.D h.a1 h.traceRep (by
    intro d hd
...
theorem contradiction (hΓ : IsMoore57 Γ) (h : D19ConcreteHypotheses Γ) : False :=
  counting_contradiction_zmod h.D (fun q => h.D_zero q) (h.Dq_le_two hΓ) h.Nd_lower
...
* 命題 2.3: $N(u)$ は $r$ の $19$-軌道 $A,B,C$ に分かれ, 反射 $t$ は $A$ を反転して
  $B,C$ を交換する.
* 補題 4.3: $B,C$-側のサイズ $38$ 軌道は $56$ 個ある.
* Section 4 の跡公式・表現論データ (`TraceRepresentationData`) と寄与公式
  $a_1(r^d)=38+38N_d$ が得られる.

補題 5.2 の局所的な 4-cycle 論証自体は `Dq_card_le_two_of_moore` で証明済みである.
さらに, 系 4.5 の下界 `Nd_lower` は `D19ConcreteHypotheses.Nd_lower` で証明する.
また, 内部差集合 `D_q` は `W` と隣接関係から `internalDiffSet` として定義する.
従ってこの公理には `D_q`, `|D_q|≤2`, `Nd_lower` を含めず, 軌道列 `W` と
Section 4 の入力データだけを残している.
-/
