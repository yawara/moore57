...
かつて外部入力として置いていた固定部分グラフ事実は, 現在では
すべて構造体外で raw 作用データから自前で構成される:

* `σ_fix : C5FixedData Γ σ` — Fix(σ) ≅ C_5.
  `aut_order_eleven_C5FixedData` (Moore57Graph/Aut/OrderElevenIsC5.lean)
  から自動的に derive される (Mačaj-Širáň 2010 Lemma 4 + 局所
  Hoffman-Singleton 整数性, axiom `srg_k_sq_plus_one_degree_classification`
  に依存). `Order22ActsOnMoore57.σ_fix` (noncomputable def).

* `τ_fix : K155FixedData Γ τ` — Fix(τ) ≅ K_{1,55}.
  `aut_involution_nonempty_K155FixedData`
  (Moore57Graph/Aut/InvolutionFixIsK155.lean) から **derive** される
  (Cameron Theorem 3.13).
...
`isMoore, σ, σ_aut, σ_pow_eleven, σ_ne_one, τ, τ_aut, τ_pow_two,
τ_ne_one, στ_relation`. 主結果 `no_Order22_acts_on_Moore57` は完全に
unconditional (Hoffman-Singleton axiom 1 個を除く).
-/
...
構造体入力は raw 作用データのみ. 固定部分グラフ事実 (`σ_fix : C5FixedData`,
`τ_fix : K155FixedData`) はそれぞれ Tier-2 の sorry-free 補題から
derive される (本ファイル下部の noncomputable def を参照).
-/
...
/-- σ-固定部分グラフが C_5: `aut_order_eleven_C5FixedData`
(Mačaj-Širáň 2010 Lemma 4 + 局所 Hoffman-Singleton; axiom
`srg_k_sq_plus_one_degree_classification` に依存) から自動的に
derive される.

旧版では構造体の外部入力フィールドとして保持していたが,
order 11 + Moore57 + ≠1 から構成可能なので, derived def に格上げ. -/
noncomputable def σ_fix : C5FixedData Γ h.σ :=
  Moore57.aut_order_eleven_C5FixedData
    h.isMoore h.σ h.σ_aut h.σ_pow_eleven h.σ_ne_one
