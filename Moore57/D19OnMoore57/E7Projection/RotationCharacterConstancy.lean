...
/-!
# Rotation-character constancy for the E₇ projection representation

Given the vertex orbit decomposition `π_V = 114·1 + 58·ε + 171·ρ` and the
eigenspace identity / dimension facts on the `E₇` projection, this file
packages the rational-character constancy property of the `E₇`
sub-representation:

* `α + β + 18·γ = 1729` (`Tr(E₇) = 1729`)
* `α − β = 33` (reflection `χ₇(t) = 33`)
* `α ≤ 113`, `β ≤ 58` (`(-8)`-eigenspace nonnegativity from the vertex split)

The remaining deep step — that for every nontrivial rotation `r^d`,
`Tr(E₇ P_{r^d}) = α + β − γ` for the SAME `α, β, γ` independent of `d` — is
the rational-character constancy of the `E₇` sub-representation.  It is
exposed here as `RotationCharacterConstancy`, to be supplied by the
natural-language Maschke / Galois argument:
  ...
  nontrivial rotations (the single Galois orbit of the 9 nontrivial rotation
  classes `{r^d, r^{-d}}`).

Once this input is provided, the multiplicity data exists and the downstream
final assembly reaches a contradiction through the `E₇` / `(-8)` projection
pipeline.
-/
...
/-- The rational-character constancy property for the E₇ projection
representation.

For every nontrivial rotation `r^d`, the matrix trace `Tr(E₇ P_{r^d})` equals
a fixed rational number `c` independent of `d`.  This is the Galois / Maschke
...
/-- The mathlib character of `h.e7ProjectionRepresentation` at every
reflection equals `33`. -/
...
/-- The mathlib character of `h.e7ProjectionRepresentation` at the identity
equals `1729`. -/
...
/-- Under the constancy input, the mathlib character of
`h.e7ProjectionRepresentation` at every nontrivial rotation equals the common
value. -/
...
/-- From the rotation-character constancy input and integer value, build the
E₇ class-character boundary.  The multiplicities are determined by

* `α − β = 33` (reflection value);
* `α + β + 18·γ = 1729` (dimension);
* `α + β − γ = intValue` (rotation constancy integer value).

The (-8)-eigenspace nonnegativity bounds `α ≤ 113`, `β ≤ 58` come from the
vertex split `π_V = 114·1 + 58·ε + 171·ρ` and must be supplied explicitly here
(they are not implied by the rotation constancy alone). -/
...
/-- Under the rotation-character constancy input and admissible multiplicities,
the representation-character components boundary exists. -/
