...
/-!
# Endpoint matching: adjacency transport and target-reflection reduction

This file unifies two layers of the endpoint-matching pipeline for the
A-fixing reflection on the source coordinate:

* **Adjacency transport** (`EndpointMatchingAFixingAdjacencyTransportBoundary`):
  a matched endpoint edge, after applying the A-fixing reflection, lands on
  the expected rotated target vertex.
* **Target-reflection reduction**
  (`EndpointMatchingAFixingTargetSignBoundary`,
  `EndpointMatchingAFixingTargetReflectionBoundary`): the formal target-vertex
  reflection identifies offset `d` with offset `-d`; the remaining
  compatibility step is to identify the `-d` endpoint target with the
  desired `d` endpoint target.

The conversion back to the existing coordinate matching equality is formal
through `AFiberCoordinates.matchingEquiv_eq_of_adj`, and the downstream
midpoint-equation, midpoint-exception, and intersection negative-offset
transports are derived from the same calculation.
-/
...
/-! ## Adjacency-transport endpoint matching -/
...
/-! ## Target-reflection-reduced endpoint matching -/
